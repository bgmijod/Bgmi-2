...
def configure_ngrok():
    ngrok_token = "2pyQu3Xzmr64P8QlWwoGCsH3HRv_2egpR3HqNNBb3w3W4yGFf"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("jony1.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in jony1.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jony1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated jony1.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update jony1.txt on VPS: {str(e)}")
